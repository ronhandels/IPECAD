...
- Start looping over individuals and within each individual loop over cycles: difficult to vectorize in R; requires double loop (100,000 individuals and ~50 annual cycles) and therefore generally slow.
- Start looping over cycles and within each cycle loop over individuals: easy to vectorize looping over individuals; requires only loops over cycles (e.g., ~50 annual cycles) and therefore generally fast.

Because loops are time-consuming in R, the second approach was preferred. This aligns well with fixed cycle times, whereas individual-by-individual approaches tend toward discrete-event simulation. For more information on vectorization, see Hadley Wickham’s Advanced R [https://adv-r.hadley.nz/subsetting.html].
