...
# @ron 
# scenario Wimo 2020
# region-specific mortality > use representative countries: https://ec.europa.eu/eurostat/web/products-eurostat-news/-/ddn-20220427-1
  # UK
  # poland
  # sweden
  # italy
  # netherlands
# optimize for each EU region (headroom analysis)



# last cycle is not half-cycle corrected. Make function for half-cycle correction yes/no (and drop last cycle after half-cycle correction / or put it to 0)
# value of information analysis

# DSA: check if there is any way to obtain 2.5th and 97.5th CI for transition probabilities from the oprobit
# costs inside and outside half-cycle correction (diagnostic based on nnt)

# @daphne/linh
# add other country life tables (csv file for each country)
# add alternative TPs natural progression (Potashman and Davis)
# add cost estimates from Linus review other EU countries (create matrix at beginnin from which to select these outcomes from)
# R markdown document
# nursing home setting 
# waning
# adaptive cycle length
# add check at beginning that age must be integer (round number, not a decimal); same for treatment duration
# add check that treatment duration should not exceed number of cycles/model time
# see if there are other checks to be made as well
# search contact details and social media of reimbursement agencies in Europe
# add side effect disutility
# add side effects costs (1 time costs in year 1); also 1 time costs for diagnostics in year 1: both not to be half-cycle corrected
# switch mciof-mcion
# outcomes in order of: QALY, COST, NHB, LY (and qaly, cost, nhb, ly)

# EXCEL
# if-statement for male/female mortality table
# flexible for end-age (same trick as PRODEMOS???)
# treatment effect
# annotate everything
# pre model analysis (PMA sheet)
# add institutionalization
...
# acknowledgment: 
  ...
  # Linh Nguyen
  # Daphne Silvertand
  # IPECAD open-source model group
  
  # dampack package developers & community
...
#setwd("C:/users/Ron/surfdrive/PhD/Projects/IPECAD/open source model/2.2 (new features)/") # set working directory; change to your own directory
setwd("D:/surfdrive/PhD/Projects/IPECAD/open source model/2.2 (new features)/")
...
l.inputs <- list(
  v.names_state = c("mciof","mcion","milof","milon","mod","sev","dth"), # disease states: mci = mild cognitive impairment; mil = mild dementia; mod = moderate dementia; sev = severe dementia; dth = dead
  ...
  p.mci_mil = 0.206, # p.x_x: transition probability between states
  p.mil_mod = 0.293, 
  p.mil_sev = 0.001, 
  p.mod_mil = 0.087, 
  p.mod_sev = 0.109, 
  ...
  hr.mort_mod = 2.419, 
  hr.mort_sev = 4.267, 
  rr.tx_mci_mil = 0.75, # treatment effect expressed as hazard ratio for specific transition (ratio between probability of dementia onset in control arm 0.208 and dementia onset in intervention arm 0.1942 of IPECAD workshop benchmark scenario) 0.194/0.208
  rr.tx_mil_mod = 0.75, # assumed same effect as for MCI to dementia
  tx_waning = 0.05, # annual waning of treatment
...
f.run_strategy <- function(l.inputs) {
  with(as.list(l.inputs), {
    ...
    m.out <- matrix(data = NA, nrow = n.cycle, ncol = 4, dimnames = list(NULL, c("ly","qaly","cost","nhb")))
...
if(T) {
  ...
  # sample transition probabilities based on oprobit
    ...
    psa_dists_params <- list(
      ...
      data.frame(value=m.TP_psa[,"p.mil_mod"], weight=rep(1,1000)),
      data.frame(value=m.TP_psa[,"p.mil_sev"], weight=rep(1,1000)),
      data.frame(value=m.TP_psa[,"p.mod_mil"], weight=rep(1,1000)),
      data.frame(value=m.TP_psa[,"p.mod_sev"], weight=rep(1,1000)),
      data.frame(value=m.TP_psa[,"p.sev_mil"], weight=rep(1,1000)),
      data.frame(value=m.TP_psa[,"p.sev_mod"], weight=rep(1,1000)),
...
# temp.l.inputs <- l.inputs
# temp.l.inputs[["p.mci_mil"]] <- 0.14/0.5
# out_base <- f.run_scenario(l.inputs = temp.l.inputs, detailed = TRUE)
# out_base[["df.out_sum"]]
# out_base[["df.out_sum"]][2,"NHB"] - out_base[["df.out_sum"]][1,"NHB"]
