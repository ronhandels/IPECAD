...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput(inputId = "n.cycle", label="Number of cycles (years) to run (should not exceed 100 when added to age)", min = 1, max = 50, value = 29), 
      sliderInput(inputId = "p.mci_mil", label = "Annual probability to transition from MCI to mild dementia", min = 0, max = 1, value = 0.23), 
      sliderInput(inputId = "p.mil_mod", label = "Annual probability to transition from mild to moderate dementia", min = 0, max = 1, value = 0.35), 
      ...
      numericInput(inputId = "p.tx_discontinuation1", label = "Annual probability to transition from on to off treatment up to period 2", min = 0, max = 1, value = 0.069), 
      sliderInput(inputId = "tx_duration", label = "maximum treatment duration (should not exceed number of cycles to run)", min = 0, max = 50, value = 29), 
      numericInput(inputId = "c.Tx", label = "Cost of treatment per cycle (year)", value = 26500 + (52/2)*78.35), 
    ...
    mainPanel(
      ...
      a("Handels et al. [2024]", href="https://doi.org/10.1101/2024.04.05.24305373"),
      helpText(
        ("Abbreviations:"), br(), 
        ("dth = death"), br(),
